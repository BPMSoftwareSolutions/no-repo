(() => {
  ...
  function renderPrimitiveBlock(block, name, lines, attrs) {
    ...
    if (block === 'action') { const label = value('label') || attrs.label || lines.join(' ').trim() || 'Action'; const endpoint = value('endpoint') || attrs.endpoint || ''; const method = value('method') || attrs.method || 'GET'; const action = value('action') || attrs.action || ''; const projectId = value('project_id') || attrs.project_id || ''; const variant = value('variant') || attrs.variant || ''; const btnClass = `loga-action${variant ? ` loga-action--${escapeHtml(variant)}` : ''}`; const actionAttr = action ? ` data-action="${escapeHtml(action)}"` : ''; const projectAttr = projectId ? ` data-project-id="${escapeHtml(projectId)}"` : ''; return `<div class="loga-action-group"><button class="${btnClass}" type="button" data-endpoint="${escapeHtml(endpoint)}" data-method="${escapeHtml(method)}"${actionAttr}${projectAttr}>${escapeHtml(label)}</button></div>`; }
    ...
    if (block === 'focus') return `<section class="loga-focus"><p class="eyebrow">${escapeHtml(value('eyebrow') || value('status') || 'focus')}</p><p class="question">${inline(value('question') || 'What matters?')}</p><p class="answer">${inline(value('answer'))}</p></section>`;
    if (block === 'surface') {
  ...
  function createRegistryModel({ block, lines, attrs, records, keyValues, value }) {
    // Telemetry-specific element handlers
    if (block === 'topbar') {
      const items = parseRecords(lines).filter(r => r.label);
      return {
        fields: {
          brand_mark: value('brand_mark'),
          title: value('title'),
    ...
    if (block === 'focus') {
      return {
        fields: {
          eyebrow: value('eyebrow') || 'Primary question',
          question: value('question'),
          answer: value('answer'),
        },
      };
    }
    if (block === 'state_grid' || block === 'event_stream') {
      return {
        items: parseRecords(lines),
        fields: keyValues,
      };
    }
    if (block === 'nav_list' || block === 'mini_section' || block === 'legend') {
      return {
        items: parseRecords(lines),
        fields: keyValues,
      };
    ...
    if (block === 'footer_note') {
      return {
        fields: {
          text: lines.join(' ').trim() || value('text'),
        },
      };
    }
    if (block === 'telemetry_app' || block === 'operator_shell') {
      return {
        fields: keyValues,
        childrenHtml: collectChildBlocks(lines).map((child) => renderBlock(child.name, child.lines, child.attrs)).join(''),
        children: true,
      };
    }
    if (block === 'rail' || block === 'main' || block === 'detail') {
      return {
        fields: keyValues,
        childrenHtml: collectChildBlocks(lines).map((child) => renderBlock(child.name, child.lines, child.attrs)).join(''),
        children: true,
